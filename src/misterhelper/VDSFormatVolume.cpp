...
BOOL VDSFormatVolume::formatByVolumeID(
	LPWSTR pwszVolumeID,
	LPWSTR pwszFileSystemTypeName,
	UINT32 ulDesiredUnitAllocationSize,
	LPWSTR pwszLabel)
{
	...
	if (pwszVolumeID == NULL || pwszFileSystemTypeName == NULL)
	{
		return result;
	}
	...
	int res = wcstombs_s(&bytesConverted, volumeID, sizeof(volumeID), pwszVolumeID, sizeof(volumeID));
	if (res != 0)
		return result;
	...
	if (true)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
				GUID volumeGUID;
				RPC_STATUS rpcRes = UuidFromString((RPC_CSTR)volumeID, &volumeGUID);

				if (rpcRes == RPC_S_OK)
				{
					// Access to volume interface via volume GUID
					hr = pService->GetObject(volumeGUID, VDS_OT_VOLUME, &pUnk);
